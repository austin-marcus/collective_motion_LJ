...
sizeInches = (7.5, 3)
dpi = 300
p = Plots.plot(p1, p2; layout=(1,2), xguide="Distance", yguide="Potential Energy", fontfamily="Computer Modern", guidefontsize=18, tickfontsize=14, colorbar_titlefontsize=18, colorbar_tickfontsize=12, dpi=dpi)
Plots.plot!(size=sizeInches .* dpi, margin=12Plots.mm)
println(p)
display(p)
...
data = filter(row->row.source=="experiment", CSV.read("cps.csv", DataFrame))
# FIGURE
...
p = Plots.plot(p1, p2; layout=(1,2), xguide="Distance", yguide="Potential Energy", fontfamily="Computer Modern", titlefontsize=22, guidefontsize=22, tickfontsize=22, dpi=300, size=(8,4).*300, margin=12Plots.mm,
                colorbar_titlefontsize=22, colorbar_tickfontsize=18)
# display(p)
savefig(p, "max_complexity2.pdf")
...
for row in eachrow(cdata)
    ...
    Plots.plot!(p1, r, x->f(x; s=row.s, d=row.d), ylim=(-4,0), label=nothing, w=width, c=cgrad(cs), line_z=row.cps, clims=clims,colorbar_title="C", colorbar_titlefontrotation=90)
    Plots.plot!(p1, r, x->LJ(x, 1, row.d), ylim=(-4,0), label=nothing, w=width, c=:black)
end
p = Plots.plot(p1; xguide="Distance", yguide="Potential Energy", fontfamily="Computer Modern", titlefontsize=22, guidefontsize=22, tickfontsize=22, dpi=300, size=(8,4).*300, margin=12Plots.mm,
                colorbar_titlefontsize=22, colorbar_tickfontsize=18)
savefig(p, "max_C_v_LJ.pdf")
...
savefig(p, "companion_potentials.pdf")
